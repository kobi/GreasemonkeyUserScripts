...
function Token(){
	...
	function showPhotoByUrl(url){
		...
		thisToken.popupImage =
			...
			.attr('src', url)
        //todo: find event that happens before 'load' but after the image resizes.
            .on('load', function(){
...
$(function(){
  ...
  imageTags.each(function(){
	...
	galleryTags.each(function(){
		try {
			...
			newToken.parentLink = $(this);
			newToken.parentTr = newToken.parentLink.parents('div.msgContainer, div.msg-title').eq(0);
			
			//in the new gallery link there could be more than one image, this takes the first one.
			var allGalleryImages = newToken.parentTr.find('.message_image_area img');
			newToken.imageUrl = allGalleryImages.attr('src');
			newToken.hasMoreImages = allGalleryImages.length > 1;
  ...
  $.each(tokens, function(i, token){
	...
	if(options.showThumbnail)
		...
		if(token.parentLink.is('img')){
			token.parentLink.after(imgThumbnail);
		}else{
